...
def test_extension_loads(duckdb_with_extension):
    ...
    ), "duck_lineage extension should appear in duckdb_extensions()"
...
def test_simple_query(sample_table, marquez_client):
    ...
    dataset = marquez_client.wait_for_dataset_with_fields(
        "duckdb_test", "memory.main.test_employees"
    )
    assert (
        dataset is not None
    ), "test_employees should be registered as a dataset in Marquez"
    ...
    assert_dataset_has_facet(
        dataset, "catalog", {"name": "memory", "type": "memory", "framework": "duckdb"}
    )
    ...
    insert_jobs = [
        j
        for j in jobs
        if "INSERT" in (j.get("name") or "")
        and "test_employees" in (j.get("name") or "")
    ]
    assert (
        insert_jobs
    ), f"No INSERT job for test_employees. Jobs: {[j.get('name') for j in jobs]}"
