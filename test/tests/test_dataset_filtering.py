...
def assert_no_dataset_in_events(events, dataset_name_substring):
    ...
    for event in events:
        for inp in get_inputs(event):
            ...
            assert (
                dataset_name_substring not in name
...
def test_exclude_prefixes_config_default(duckdb_with_extension):
    ...
    value = conn.execute("SELECT current_setting('duck_lineage_exclude_dataset_prefixes')").fetchone()[0]
...
def test_exclude_prefixes_config_multiple(duckdb_with_extension):
    ...
    conn = duckdb_with_extension
    conn.execute("SET duck_lineage_exclude_dataset_prefixes = 'prefix_a_, prefix_b_,prefix_c_'")
    value = conn.execute("SELECT current_setting('duck_lineage_exclude_dataset_prefixes')").fetchone()[0]
...
@pytest.mark.integration
def test_excluded_prefix_table_not_in_inputs(lineage_connection, marquez_client, clean_marquez_namespace):
    """Tables matching exclude prefix should not appear as inputs in events."""
    conn = lineage_connection
    ns = clean_marquez_namespace

    conn.execute("SET duck_lineage_exclude_dataset_prefixes = 'memory.main.__excluded_'")
    ...
    assert any_dataset_in_events(events, "normal_source"), "normal_source should appear in events"
...
@pytest.mark.integration
def test_non_excluded_table_appears_normally(lineage_connection, marquez_client, clean_marquez_namespace):
    ...
    assert any_dataset_in_events(events, "normal_table"), "normal_table should appear in events with default filtering"
...
@pytest.mark.integration
def test_custom_prefix_filters_matching_tables(lineage_connection, marquez_client, clean_marquez_namespace):
    """Custom prefix should filter matching tables and keep non-matching ones."""
    conn = lineage_connection
    ns = clean_marquez_namespace

    conn.execute("SET duck_lineage_exclude_dataset_prefixes = 'memory.main.temp_staging_'")
    ...
    assert any_dataset_in_events(events, "production_data"), "production_data should appear"
    assert_no_dataset_in_events(events, "temp_staging_data")


@pytest.mark.integration
def test_multiple_prefixes_all_filter(lineage_connection, marquez_client, clean_marquez_namespace):
...
@pytest.mark.integration
def test_empty_prefix_disables_filtering(lineage_connection, marquez_client, clean_marquez_namespace):
    ...
    assert any_dataset_in_events(
        events, "__ducklake_metadata_test"
    ), "With empty prefix, __ducklake_metadata_test should appear in events"


@pytest.mark.integration
def test_prefix_is_case_sensitive(lineage_connection, marquez_client, clean_marquez_namespace):
    ...
    conn.execute("SET duck_lineage_exclude_dataset_prefixes = 'memory.main.__ducklake_metadata_'")
...
@pytest.mark.integration
def test_prefix_match_is_starts_with_not_contains(lineage_connection, marquez_client, clean_marquez_namespace):
    ...
    assert any_dataset_in_events(
        events, "data__excluded_test"
    ), "Table with prefix in middle of name should not be filtered"


@pytest.mark.integration
def test_event_skipped_when_all_datasets_filtered(lineage_connection, marquez_client, clean_marquez_namespace):
    """Events with no remaining datasets after filtering should not be emitted."""
    conn = lineage_connection
    ns = clean_marquez_namespace

    conn.execute("SET duck_lineage_exclude_dataset_prefixes = 'memory.main.__filtered_'")
    ...
    filtered_jobs = [j for j in jobs if "__filtered_only" in (j.get("name") or "")]
    assert not filtered_jobs, (
        f"Fully filtered operations should not create jobs, but found: " f"{[j.get('name') for j in filtered_jobs]}"
    )


@pytest.mark.integration
def test_mixed_filtered_and_unfiltered_inputs(lineage_connection, marquez_client, clean_marquez_namespace):
    """When some inputs are filtered and others aren't, only unfiltered appear."""
    conn = lineage_connection
    ns = clean_marquez_namespace

    conn.execute("SET duck_lineage_exclude_dataset_prefixes = 'memory.main.__hidden_'")
    ...
    ctas_start_events = [
        e
        for e in events
        if e.get("eventType") == "START" and any("joined" in (o.get("name") or "") for o in get_outputs(e))
    ]
    ...
    for e in ctas_start_events:
        ...
        assert any("visible_src" in n for n in input_names), f"visible_src should be in inputs. Found: {input_names}"
        # __hidden_src should NOT be in inputs
        assert all(
            "__hidden_src" not in n for n in input_names
...
@pytest.mark.integration
def test_insert_into_excluded_table_filtered(lineage_connection, marquez_client, clean_marquez_namespace):
    """INSERT into an excluded table should not generate lineage events."""
    conn = lineage_connection
    ns = clean_marquez_namespace

    conn.execute("SET duck_lineage_exclude_dataset_prefixes = 'memory.main.__skip_'")
    ...
    assert not skip_jobs, (
        f"INSERT into excluded table should not create jobs, found: " f"{[j.get('name') for j in skip_jobs]}"
    )


@pytest.mark.integration
def test_drop_excluded_table_filtered(lineage_connection, marquez_client, clean_marquez_namespace):
    """DROP of an excluded table should not generate lineage events."""
    conn = lineage_connection
    ns = clean_marquez_namespace

    conn.execute("SET duck_lineage_exclude_dataset_prefixes = 'memory.main.__drop_'")
    ...
    assert not drop_jobs, (
        f"DROP of excluded table should not create jobs, found: " f"{[j.get('name') for j in drop_jobs]}"
    )


@pytest.mark.integration
def test_update_excluded_table_filtered(lineage_connection, marquez_client, clean_marquez_namespace):
    """UPDATE of an excluded table should not generate lineage events for the update."""
    conn = lineage_connection
    ns = clean_marquez_namespace

    conn.execute("SET duck_lineage_exclude_dataset_prefixes = 'memory.main.__upd_'")
    ...
    assert not upd_jobs, (
        f"UPDATE of excluded table should not create jobs, found: " f"{[j.get('name') for j in upd_jobs]}"
    )
