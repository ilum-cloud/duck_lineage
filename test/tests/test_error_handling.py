...
def test_special_characters_in_names(lineage_connection, marquez_client):
    ...
    dataset = marquez_client.wait_for_dataset_with_fields(
        namespace, "memory.main.my-special-table"
    )
    assert (
        dataset is not None
    ), "Dataset for 'my-special-table' should be registered in Marquez."
...
def test_very_long_query(lineage_connection, marquez_client):
    ...
    dataset = marquez_client.wait_for_dataset_with_fields(
        namespace, "memory.main.wide_table"
    )
    assert (
        dataset is not None
    ), "Dataset for 'wide_table' should be registered in Marquez."
    ...
    assert_dataset_has_facet(dataset, "schema")
    assert_dataset_has_facet(
        dataset, "catalog", {"type": "memory", "framework": "duckdb"}
    )
...
def test_transaction_rollback(lineage_connection, marquez_client):
    ...
    assert (
        dataset is not None
    ), "accounts dataset should exist in Marquez despite the rollback"
...
def test_null_values_handling(lineage_connection, marquez_client):
    ...
    source = marquez_client.wait_for_dataset_with_fields(
        namespace, "memory.main.nullable_data"
    )
    assert (
        source is not None
    ), "Source dataset 'nullable_data' should be registered in Marquez."
    ...
    derived = marquez_client.wait_for_dataset_with_fields(
        namespace, "memory.main.filtered_data"
    )
    assert (
        derived is not None
    ), "Derived dataset 'filtered_data' should be registered in Marquez."
    ...
    assert_dataset_has_facet(
        derived, "catalog", {"type": "memory", "framework": "duckdb"}
    )
...
def test_configuration_persistence(extension_path, marquez_api_url, marquez_client):
    ...
    assert (
        ds2 is not None
    ), "test2 should be tracked — config should persist across queries"
    ...
    ddl_jobs = [
        j
        for j in jobs
        if any(kw in (j.get("name") or "") for kw in ("CREATE", "INSERT"))
    ]
    assert (
        len(ddl_jobs) >= 2
