...
def test_csv_read_lineage(lineage_connection, marquez_client, tmp_path):
    ...
    output = marquez_client.wait_for_dataset_with_fields(
        namespace, "memory.main.imported_data"
    )
    assert (
        output is not None
    ), "Output table 'imported_data' should be registered in Marquez"

    assert_valid_dataset(output, namespace, "imported_data")
    assert_dataset_has_fields(
        output, {"id": "BIGINT", "name": "VARCHAR", "value": "BIGINT"}
    )
    assert_dataset_has_facet(output, "datasetType", {"datasetType": "TABLE"})
    assert_dataset_has_facet(
        output, "catalog", {"type": "memory", "framework": "duckdb"}
    )
...
def test_csv_write_lineage(lineage_connection, marquez_client, tmp_path):
    ...
    source = marquez_client.wait_for_dataset_with_fields(
        namespace, "memory.main.export_data"
    )
    assert (
        source is not None
    ), "Source table 'export_data' should be registered in Marquez"
    assert_valid_dataset(source, namespace, "export_data")
    assert_dataset_has_fields(
        source, {"id": "INTEGER", "name": "VARCHAR", "score": "DECIMAL"}
    )
...
def test_parquet_read_lineage(lineage_connection, marquez_client, tmp_path):
    ...
    setup_conn.execute(
        "INSERT INTO temp_data VALUES (1, 'Alpha', 10.5), (2, 'Beta', 20.7)"
    )
    ...
    assert (
        pq_dataset is not None
    ), f"Should have created dataset for Parquet file in 'file' namespace"
    assert pq_dataset.get("namespace") == FILE_NAMESPACE

    # Validate output table dataset
    output = marquez_client.wait_for_dataset_with_fields(
        namespace, "memory.main.from_parquet"
    )
    assert (
        output is not None
    ), "Output table 'from_parquet' should be registered in Marquez"

    assert_valid_dataset(output, namespace, "from_parquet")
    assert_dataset_has_fields(
        output, {"id": "INTEGER", "name": "VARCHAR", "value": "DOUBLE"}
    )
    assert_dataset_has_facet(output, "datasetType", {"datasetType": "TABLE"})
...
def test_parquet_write_lineage(lineage_connection, marquez_client, tmp_path):
    ...
    conn.execute(
        "INSERT INTO analytics_results VALUES ('conversion_rate', 0.125, '2024-01-01 10:00:00')"
    )
    ...
    source = marquez_client.wait_for_dataset_with_fields(
        namespace, "memory.main.analytics_results"
    )
...
def test_multiple_file_operations(lineage_connection, marquez_client, tmp_path):
    ...
    processed = marquez_client.wait_for_dataset_with_fields(
        namespace, "memory.main.processed_data"
    )
    ...
    assert_dataset_has_fields(
        processed, {"category": "VARCHAR", "count": "BIGINT", "total_amount": "HUGEINT"}
    )
    assert_dataset_has_facet(processed, "schema")
    assert_dataset_has_facet(processed, "datasetType", {"datasetType": "TABLE"})
    assert_dataset_has_facet(
        processed, "catalog", {"type": "memory", "framework": "duckdb"}
    )
    ...
    etl_jobs = [
        j
        for j in jobs
        if any(kw in (j.get("name") or "") for kw in ("CREATE_TABLE", "COPY"))
    ]
    ...
    ctas_jobs = [
        j
        for j in etl_jobs
        if "processed_data" in (j.get("name") or "")
        and "raw_data" in (j.get("name") or "")
    ]
    if ctas_jobs:
        ctas = ctas_jobs[0]
        assert_job_has_io(
