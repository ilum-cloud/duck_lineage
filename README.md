...
This extension, Openlineage, allow you to ... <extension_goal>.
...
Now we can use the features from the extension directly in DuckDB. The template contains a single scalar function `openlineage()` that takes a string arguments and returns a string:
```
D select openlineage('Jane') as result;
┌───────────────┐
│    result     │
│    varchar    │
├───────────────┤
